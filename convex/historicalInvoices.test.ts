...
const createHistorical = (
  api as unknown as {
    historicalInvoices: {
      ...
      createUnpaid: FunctionReference<
        "mutation",
        "public",
        UnpaidArgs,
        Id<"invoices">
...
const renumberOutstanding = (
  internal as unknown as {
    historicalInvoices: {
      renumberOutstanding: FunctionReference<
        "mutation",
        "internal",
        { dryRun: boolean; confirm?: string },
        unknown
      >;
    };
  }
).historicalInvoices.renumberOutstanding;
const correctOutstandingDueDates = (
  internal as unknown as {
    historicalInvoices: {
      correctOutstandingDueDates: FunctionReference<
        "mutation",
        "internal",
        { dryRun: boolean; confirm?: string },
        unknown
...
async function seed(t: ReturnType<typeof convexTest>) {
  return await t.run(async (ctx) => {
    ...
    const userId = await ctx.db.insert("users", {
      name: "Finance CEO",
      tokenIdentifier: "historical-test",
      role: "ceo",
    });
...
describe("historical paid invoices", () => {
  ...
  it("requires all four exact targets and fails closed on every target guard", async () => {
    const invalidCases: Array<{
      ...
      {
        name: "wrong invoice number",
        overrides: { invoiceNumber: "INV-2026-99999" },
      },
      { name: "wrong company", overrides: { companyName: "Wrong Company" } },
      {
        name: "wrong reference",
        overrides: { originalReference: "WRONG-REFERENCE" },
      },
    ...
    for (const invalidCase of invalidCases) {
      ...
      await expect(
        ...
        "not an eligible historical description correction target",
      );
      expect(testContext.patches).toHaveLength(0);
    }

    const missingTargetContext = createDescriptionTestContext(
      DESCRIPTION_TARGETS.slice(1).map((target) =>
        makeDescriptionInvoice(target),
      ),
    );
    await expect(
  ...
  it("records a paid multi-month invoice with exact existing money semantics", async () => {
    ...
    const invoiceId = await user.mutation(createHistorical, {
      ...
      originalReference: "S00065",
      invoiceDate: "2026-05-14",
      coverageStartMonth: "2026-05",
      monthsCovered: 3,
      monthlyAmount: 5874.93,
      itemDescription: "Managed cloud services",
      paymentDate: "2026-05-14",
      receivingAccountId: s.accountId,
      transactionId: "ODOO-S00065",
      paymentReference: "Paid in Odoo",
    });
    ...
    expect(result.invoice).toMatchObject({
      ...
      sourceSystem: "odoo",
      originalReference: "S00065",
      issueDate: Date.UTC(2026, 4, 14),
      dueDate: Date.UTC(2026, 4, 14),
      status: "paid",
      grandTotal: 17624.79,
      grandTotalCents: 1762479,
      amountPaid: 17624.79,
      amountPaidCents: 1762479,
      balanceDue: 0,
      balanceDueCents: 0,
      historicalCoverageStartMonth: "2026-05",
      historicalCoverageMonths: 3,
    });
    expect(result.invoice?.invoiceNumber).toBe(
      `INV-${new Date().getUTCFullYear()}-00001`,
    );
    expect(result.invoice?.revenueAllocations).toEqual([
      { month: "2026-05", amount: 5874.93 },
      { month: "2026-06", amount: 5874.93 },
      { month: "2026-07", amount: 5874.93 },
    ]);
    expect(result.invoice?.lineItems[0]?.itemName).toBe(
  ...
  it("uses the shared normal sequence for historical and subsequent CRM invoices", async () => {
    ...
    const paidId = await user.mutation(createHistorical, {
      companyId: s.companyId,
      originalReference: "SEQUENCE-PAID",
      ...
      receivingAccountId: s.accountId,
      transactionId: "SEQUENCE-PAID",
    });
    const unpaidId = await user.mutation(createHistoricalUnpaid, {
      companyId: s.companyId,
      originalReference: "SEQUENCE-UNPAID",
      invoiceDate: "2026-01-02",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 100,
    });
    const normalDraftId = await t.run(async (ctx) =>
    ...
    await user.mutation(api.invoices.issueInvoice, {
      invoiceId: normalDraftId,
    });
    const result = await t.run(async (ctx) => ({
  ...
  it("previews and executes only outstanding legacy historical renumbering", async () => {
    ...
    const partialId = await user.mutation(createHistoricalUnpaid, {
      companyId: s.companyId,
      originalReference: "LEGACY-PARTIAL",
      invoiceDate: "2026-01-02",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 100,
    ...
    await user.mutation(api.invoices.recordPayment, {
      invoiceId: partialId,
      amount: 20,
      paidAt: Date.UTC(2026, 0, 4),
      receivingAccountId: s.accountId,
      transactionId: "LEGACY-PARTIAL-1",
    });
    await t.run(async (ctx) => {
      await ctx.db.patch(paidId, {
        invoiceNumber: "HIST-ODOO-PAID",
        status: "paid",
      });
    ...
    await expect(
      t.mutation(renumberOutstanding, { dryRun: false }),
    ).rejects.toThrow("Exact confirmation required");
    ...
    expect(preview).toEqual([
      expect.objectContaining({
        invoiceId: earlyId,
        oldInvoiceNumber: "HIST-ODOO-EARLY",
        newInvoiceNumber: `INV-${year}-00004`,
        status: "issued",
        grandTotal: 100,
        amountPaid: 0,
        balanceDue: 100,
      ...
      expect.objectContaining({
        invoiceId: partialId,
        oldInvoiceNumber: "HIST-ODOO-PARTIAL",
        newInvoiceNumber: `INV-${year}-00005`,
    ...
    await t.mutation(renumberOutstanding, {
      dryRun: false,
      confirm: "RENUMBER_OUTSTANDING_HISTORICAL_INVOICES",
    });
    ...
    expect(after.invoices.find((invoice) => invoice._id === paidId)).toEqual(
      before.invoices.find((invoice) => invoice._id === paidId),
    );
    expect(after.invoices.find((invoice) => invoice._id === earlyId)).toEqual({
      ...before.invoices.find((invoice) => invoice._id === earlyId),
      invoiceNumber: `INV-${year}-00004`,
    });
    expect(after.invoices.find((invoice) => invoice._id === partialId)).toEqual(
      {
        ...before.invoices.find((invoice) => invoice._id === partialId),
        invoiceNumber: `INV-${year}-00005`,
      },
    );
  });
  ...
  it("keeps historical migration above existing valid invoice numbers", async () => {
    ...
    const targetId = await user.mutation(createHistoricalUnpaid, {
      companyId: s.companyId,
      originalReference: "COLLISION-TARGET",
      invoiceDate: "2026-01-01",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 100,
    ...
    const occupiedId = await user.mutation(createHistoricalUnpaid, {
      companyId: s.companyId,
      originalReference: "COLLISION-OCCUPIED",
      invoiceDate: "2026-01-02",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 100,
    ...
    await t.run(async (ctx) => {
      await ctx.db.patch(targetId, {
        invoiceNumber: "HIST-ODOO-COLLISION-TARGET",
      });
      await ctx.db.patch(occupiedId, {
        invoiceNumber: occupiedNumber,
        status: "paid",
      });
    });
    const preview = await t.mutation(renumberOutstanding, { dryRun: true });
    ...
    const result = await t.run(async (ctx) => ({
      ...
      occupied: await ctx.db.get(occupiedId),
    }));
  ...
  it("corrects only outstanding historical due dates in a dry run and execution", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.patch(zeroBalanceId, {
        dueDate: Date.UTC(2026, 0, 3),
        amountPaid: 100,
        balanceDue: 0,
    ...
    await expect(
      t.mutation(correctOutstandingDueDates, { dryRun: false }),
    ).rejects.toThrow("Exact confirmation required");
    const preview = await t.mutation(correctOutstandingDueDates, {
      dryRun: true,
    });
    expect(preview).toEqual([
      expect.objectContaining({
      ...
      expect.objectContaining({
        invoiceId: overdueId,
        status: "overdue",
        issueDate: Date.UTC(2026, 0, 4),
        oldDueDate: Date.UTC(2026, 0, 4),
        newDueDate: Date.UTC(2026, 0, 11),
        balanceDue: 100,
        companyId: s.companyId,
        paymentTermDays: 7,
      }),
    ...
    await t.mutation(correctOutstandingDueDates, {
      dryRun: false,
    ...
    expect(
      after.invoices.find((invoice) => invoice._id === zeroBalanceId),
    ).toEqual(before.invoices.find((invoice) => invoice._id === zeroBalanceId));
    expect(after.invoices.find((invoice) => invoice._id === issuedId)).toEqual({
      ...before.invoices.find((invoice) => invoice._id === issuedId),
      dueDate: Date.UTC(2026, 0, 8),
    });
    expect(after.invoices.find((invoice) => invoice._id === overdueId)).toEqual(
      {
        ...before.invoices.find((invoice) => invoice._id === overdueId),
        dueDate: Date.UTC(2026, 0, 11),
      },
    );
  });
  ...
  it("reuses the existing overdue marker and payment flow for historical invoices", async () => {
    ...
    const overdueResult = await t.mutation(
  ...
  it("allows the normal payment flow to partially and fully pay an unpaid historical invoice", async () => {
    ...
    await user.mutation(api.invoices.recordPayment, {
      invoiceId,
      amount: 40,
      paidAt: Date.UTC(2026, 1, 5),
      method: "Bank Transfer",
      receivingAccountId: s.accountId,
      transactionId: "PAY-1-A",
    });
  ...
  it("guards duplicates while allowing the same reference for another customer", async () => {
    ...
    const args = {
      companyId: s.companyId,
      originalReference: "INV/2026/00045",
      invoiceDate: "2026-01-04",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 100,
    ...
    await expect(user.mutation(createHistorical, args)).rejects.toThrow(
      "already exists",
    );
    await expect(
      user.mutation(createHistorical, {
        ...args,
        originalReference: "OTHER-1",
        transactionId: "DUPLICATE-2",
      }),
  ...
  it("keeps normal records and validates receiving accounts", async () => {
    ...
    const before = await t.run(async (ctx) => ({
      ...
      accounts: await ctx.db.query("receivingAccounts").collect(),
    }));
    await user.mutation(createHistorical, {
      companyId: s.companyId,
      originalReference: "SAFE-1",
      invoiceDate: "2026-01-01",
      coverageStartMonth: "2026-01",
      monthsCovered: 1,
      monthlyAmount: 10,
    ...
    expect(after.accounts.map((account) => account._id)).toEqual(
      before.accounts.map((account) => account._id),
    );
  });
