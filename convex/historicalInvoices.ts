...
async function getCurrentUserOrThrow(
  ...
  if (user.role === "monitoring") {
    throw new ConvexError({
      code: "FORBIDDEN",
      message: "Monitoring users cannot access Finance",
    });
  }
  return user;
}
...
function normalizeReference(reference: string) {
  ...
  if (!normalized) {
    throw new ConvexError({
      code: "BAD_REQUEST",
      message: "Original reference is required",
    });
  }
  return normalized;
}
...
function coverageMonths(startMonth: string, count: number) {
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(startMonth)) {
    throw new ConvexError({
      code: "BAD_REQUEST",
      message: "Coverage start month must use YYYY-MM",
    });
  }
  if (!Number.isInteger(count) || count < 1) {
    throw new ConvexError({
      code: "BAD_REQUEST",
      message: "Months covered must be at least 1",
    });
  }
  const start =
    Number(startMonth.slice(0, 4)) * 12 + Number(startMonth.slice(5, 7)) - 1;
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      (!receivingAccount.countryId ||
        receivingAccount.countryId !== company.countryId)
    ) {
      throw new ConvexError({
        code: "BAD_REQUEST",
    ...
    if (receivingAccount && receivingAccount.currency !== "USD") {
      throw new ConvexError({
        code: "BAD_REQUEST",
        message: "Payment account currency must be USD",
      });
    }
    if (receivingAccount) {
